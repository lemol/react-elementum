...
const buildCreateMutation = ({ createName, fields, typeName, inputTypeName }) => gql`
...
const handleSave = ({
  ...
  try {
    ...
    if (onSuccess) {
      onSuccess({ item, result: data && data[createName], bag });
    }
  } catch (error) {
...
export default ({
  ...
  return (initialValue) => {
    ...
    const mutation = useMutation(CREATE_MUTATION, {
  ...
  const Wrapper = adopt(
    {
      props: ({ bag, render }) => <Value initial={{ bag }}>{({ value }) => render(value)}</Value>,
      createMutation: ({ render }) => (
        <Mutation mutation={CREATE_MUTATION} refetchQueries={() => [...refetchQueries]}>
          {(mutation, data) => render({ mutation, data })}
        </Mutation>
      ),
    },
    ({ props: { bag }, createMutation }) => {
      const {
        mutation,
        data: { loading: saving, error: createMutationError },
      } = createMutation;
      let error = null;

      if (createMutationError) {
        error = { ...(error || {}), createMutationError };
      }
