...
const getModalDataQuery = ({ path, queryName }) => gql`
  query ${queryName} {
    ${path[0]} @client {
      ${buildQuery({ path: drop(path, 1), fields: dataFields })}
    }
  }
`;

const getOpenModalMutation = ({ name, openModal }) => gql`
  mutation Open${name}($itemId: ID!) {
    ${openModal}(itemId: $itemId) @client
  }
`;

const getCloseModalMutation = ({ name, closeModal }) => gql`
  mutation Close${name} {
    ${closeModal} @client
  }
`;
...
export const modal = ({ name }) => {
  ...
  const Wrapper = adopt(
    {
      props: ({ itemId, render }) => (
        <Value initial={{ itemId }}>{({ value }) => render(value)}</Value>
      ),
      close: ({ render }) => <Mutation mutation={CLOSE_MODAL_MUTATION}>{render}</Mutation>,
      open: ({ itemId, render }) => (
        <Mutation mutation={OPEN_MODAL_MUTATION} variables={{ itemId: itemId || null }}>
          {render}
        </Mutation>
      ),
      query: ({ render }) => <Query query={MODAL_DATA_QUERY}>{render}</Query>,
    },
    ({ close, open, props: { itemId }, query: { error, loading, data } }) => {
      if (error) {
        return { error };
      }
...
export const resolvers = ({ name }) => {
  const options = buildOptions({ name });

  const openModal = (_, { itemId }, { cache }) => {
